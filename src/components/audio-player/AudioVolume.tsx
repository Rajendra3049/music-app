...
export function AudioVolume({
  ...
  return (
    <div 
      className={`flex items-center gap-3 ml-auto bg-white/5 ${getContainerPadding()} rounded-full backdrop-blur-sm border border-white/10`}
      onWheel={handleVolumeWheel}
    >
      <motion.button
        className={`${getButtonPadding()} rounded-full hover:bg-white/10 text-gray-300 hover:text-white transition-colors`}
        ...
        <div className="w-20 group relative">
          ...
          <style jsx>{`
            input[type='range']::-webkit-slider-thumb {
              ...
              appearance: none;
              width: ${size === 'sm' ? '10px' : size === 'lg' ? '14px' : '12px'};
              height: ${size === 'sm' ? '10px' : size === 'lg' ? '14px' : '12px'};
              border-radius: 50%;
              background: #fff;
              cursor: pointer;
